...
contract Tracking{
    ...
    uint256 public shipmentCount;//by default initialise to zero
    ...
    TypeShipment[] typeShipments;//entire typeshipments data
    ...
    event ShipmentPaid(address indexed sender ,address indexed receiver ,uint256 amount);
     ...
     function createShipment(address _receiver,uint256 _pickupTime,uint256 _distance,uint256 _price) public payable{
        ...
        0, _price, _distance, ShipmentStatus.PENDING, false);
     ...
     function startShipment(address _sender,address _receiver,uint256 _index) public{
        ...
        typeShipments.status = ShipmentStatus.IN_TRANSIT;
     ...
     function CompleteShipment(address _sender,address _receiver,uint256 _index) public  {
        ...
        require(shipment.receiver == _receiver,"Invalid recceiver");
       ...
       payable(shipment.sender).transfer(shipment.price);//instead of amount
       shipment.isPaid = true;
       typeShipment.isPaid = true;
       
      emit  ShipmentDelivered( _sender, _receiver, shipment.deliveryTime);
      emit ShipmentPaid(_sender,_receiver,shipment.price);//instead of amount
     }

     function getShipment(address _sender,uint256 _index)  public view returns(address,
     address,uint256,uint256,uint256,uint256,Shipment.status,bool){
      Shipment memory  shipment = shipments[_sender][_index];
